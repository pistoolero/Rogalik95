...
function updateCounter() {
    ...
    if(inner_tick > tick_count){
        inner_tick = 0;
        $.ajax({
            url: "/channel/subscriberCount",
            dataType: "json",
            success: function (e) {
                console.log(e.subscriberCount);
                $(".odometer").html(e.subscriberCount);
            }
        })
    }
}
...
$(function(){
    ...
    $.getJSON('https://www.googleapis.com/youtube/v3/search?order=date&part=snippet&channelId=' + cid + '&maxResults=1&key=AIzaSyA5NQq-oSCOeBzBDq_CD3NlhQ4wUzM4MSM', function (data) {
       var title = data.items[0].snippet.title; var videoid = data.items[0].id.videoId;
